...
public void removeOriginalDirectory(Path directoryToDelete) {
    ...
    public Path zipConvertedFiles(String sessionId, String tempDirName, String sourceBoard, String targetBoard) throws IOException {
        ...
        logger.info("** Zipping directory: " + targetSubDirPath);
        ...
        try (
            ...
            Path sourceDirPath = Paths.get(targetSubDirPath);
            ...
            Files.walk(sourceDirPath)
                ...
                .forEach(file -> {
                    try {
                        ...
                        String modifiedEntryName = relativePath.toString().replaceFirst("^([^/\\\\]+)", "$1_" + targetBoard);
                        ZipEntry zipEntry = new ZipEntry(modifiedEntryName);
                        zos.putNextEntry(zipEntry);
                        Files.copy(file, zos);
                        zos.closeEntry();
                    } catch (IOException e) {
                        logger.error(ANSI_RED + "Error zipping file: " + file + ANSI_RESET, e);
                    }
                });

        } catch (IOException e) {
            throw new IOException(ANSI_RED + "Error encountered during zip operation: " + e.getMessage() + ANSI_RESET, e);
        }
